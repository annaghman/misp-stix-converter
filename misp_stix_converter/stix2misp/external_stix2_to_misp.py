...
class ExternalSTIX2toMISPParser(STIX2toMISPParser, ExternalSTIXtoMISPParser):
    def __init__(self):
        ...
        self._oca_parser: ExternalSTIX2OCAConverter
    ...
    @property
    def oca_parser(self) -> ExternalSTIX2OCAConverter:
        if not hasattr(self, '_oca_parser'):
            self._oca_parser = ExternalSTIX2OCAConverter(self)
        return self._oca_parser
    ...
    def _load_oca_behavior(self, oca_behavior):
        self._check_uuid(oca_behavior.id if hasattr(oca_behavior, 'id') else oca_behavior['id'])
        try:
            self._x_oca_behavior[oca_behavior.id if hasattr(oca_behavior, 'id') else oca_behavior['id']] = oca_behavior
        except AttributeError:
            self._x_oca_behavior = {oca_behavior.id if hasattr(oca_behavior, 'id') else oca_behavior['id']: oca_behavior}

    def _load_oca_detection(self, oca_detection):
        self._check_uuid(oca_detection.id if hasattr(oca_detection, 'id') else oca_detection['id'])
        try:
            self._x_oca_detection[oca_detection.id if hasattr(oca_detection, 'id') else oca_detection['id']] = oca_detection
        except AttributeError:
            self._x_oca_detection = {oca_detection.id if hasattr(oca_detection, 'id') else oca_detection['id']: oca_detection}
