...
@CustomObject(
    'x-oca-detection',
    [
        ('id', IDProperty('x-oca-detection', spec_version='2.1')),
        ('labels', ListProperty(StringProperty, required=True)),
        ('created', TimestampProperty(required=True, precision='millisecond')),
        ('modified', TimestampProperty(required=True, precision='millisecond')),
        ('created_by_ref', ReferenceProperty(valid_types='identity', spec_version='2.1')),
        ('object_marking_refs', ListProperty(ReferenceProperty(valid_types='marking-definition', spec_version='2.1'))),
        ('name', StringProperty(required=True)),
        ('detection_type', StringProperty(required=True)),
        ('logic', StringProperty(required=True))
    ]
)
class OCADetection:
    pass
...
class MISPtoSTIX21Parser(MISPtoSTIX2Parser):
    ...
    def _parse_oca_behavior_object(self, misp_object: Union[MISPObject, dict]):
        """Parse MISP oca-behavior object to STIX x-oca-behavior object."""
        behavior_id = self._parse_stix_object_id(
            'object', 'x-oca-behavior', misp_object
        )
        timestamp = self._datetime_from_timestamp(misp_object['timestamp'])
        behavior_args = {
            'id': behavior_id,
        ...
        if misp_object.get('comment'):
            behavior_args['description'] = misp_object['comment']
        
        # Handle markings
        markings = self._handle_object_tags_and_galaxies(
            misp_object, behavior_id, behavior_args['modified']
        )
        if markings:
            self._handle_markings(behavior_args, markings)
        ...
        if 'name' in attributes:
            behavior_args['name'] = attributes['name'][0] if isinstance(attributes['name'], list) else attributes['name']
        if 'technique-id' in attributes:
            behavior_args['behavior_type'] = attributes['technique-id'][0] if isinstance(attributes['technique-id'], list) else attributes['technique-id']
        
        # Handle external references for ATT&CK
        if 'technique-id' in attributes:
            technique_id = attributes['technique-id'][0] if isinstance(attributes['technique-id'], list) else attributes['technique-id']
            external_refs = [{
                'source_name': 'mitre-attack',
                'external_id': technique_id,
                'url': f'https://attack.mitre.org/techniques/{technique_id}/'
            }]
            behavior_args['external_references'] = external_refs
        
        # Create and append the STIX object
        stix_object = OCABehavior(**behavior_args)
        self._append_SDO(stix_object)
        
        # Handle object references (convert MISP references to STIX relationships)
        if misp_object.get('ObjectReference'):
            self._parse_object_relationships(
                misp_object['ObjectReference'], behavior_id,
                behavior_args['modified']
            )

    def _parse_oca_detection_object(self, misp_object: Union[MISPObject, dict]):
        """Parse MISP oca-detection object to STIX x-oca-detection object."""
        detection_id = self._parse_stix_object_id(
            'object', 'x-oca-detection', misp_object
        )
        timestamp = self._datetime_from_timestamp(misp_object['timestamp'])
        detection_args = {
            'id': detection_id,
        ...
        # Handle object description
        if misp_object.get('comment'):
            detection_args['description'] = misp_object['comment']
        
        # Handle markings
        markings = self._handle_object_tags_and_galaxies(
            misp_object, detection_id, detection_args['modified']
        )
        if markings:
            self._handle_markings(detection_args, markings)
        ...
        # Map key attributes
        if 'name' in attributes:
            detection_args['name'] = attributes['name'][0] if isinstance(attributes['name'], list) else attributes['name']
        if 'detection-type' in attributes:
            detection_args['detection_type'] = attributes['detection-type'][0] if isinstance(attributes['detection-type'], list) else attributes['detection-type']
        if 'detection-logic' in attributes:
            detection_args['logic'] = attributes['detection-logic'][0] if isinstance(attributes['detection-logic'], list) else attributes['detection-logic']
        
        # Create and append the STIX object
        stix_object = OCADetection(**detection_args)
        self._append_SDO(stix_object)
        
        # Handle object references (convert MISP references to STIX relationships)
        if misp_object.get('ObjectReference'):
            self._parse_object_relationships(
                misp_object['ObjectReference'], detection_id,
                detection_args['modified']
            )
